...
let MOCK_EVENTOS: EventoCarga[] = [
  ...
  },
  {
    ...
    geolocalizacion: { lat: -33.460, lon: -70.658 },
...
let MOCK_EVIDENCIAS: Evidencia[] = [
  { id: 'ev1', id_evento: 'evt001', tipo: 'foto_surtidor', url: 'https://via.placeholder.com/150/0000FF/FFFFFF?text=SURTIDOR', descripcion: 'Foto del surtidor', fecha: new Date().toISOString() },
  { id: 'ev2', id_evento: 'evt001', tipo: 'foto_odometro', url: 'https://via.placeholder.com/150/FF0000/FFFFFF?text=ODOMETRO', descripcion: 'Foto del odómetro', fecha: new Date().toISOString() },
  { id: 'ev3', id_evento: 'evt002', tipo: 'foto_surtidor', url: 'https://via.placeholder.com/150/00FF00/FFFFFF?text=SURTIDOR_EXC', descripcion: 'Foto del surtidor de la excavadora', fecha: new Date().toISOString() },
];

let MOCK_ALERTAS: Alerta[] = [
  { id: 'ale1', id_empresa: 'emp1', tipo: 'ubicacion_invalida', mensaje: 'Carga fuera de la zona permitida', id_evento: 'evt002', fecha: new Date().toISOString(), resuelta: false },
  { id: 'ale2', id_empresa: 'emp1', tipo: 'desvio_consumo', mensaje: 'Consumo inusualmente alto para veh1', id_evento: 'evt001', fecha: new Date().toISOString(), resuelta: false },
];

let MOCK_AUDITLOGS: AuditLog[] = [
  { id: 'aud1', id_empresa: 'emp1', id_usuario: 'user2', accion: 'CREATE_VEHICULO', entidad: 'Vehiculo', id_entidad: 'veh1', fecha: new Date().toISOString(), detalles: { patente: 'ABCD12', nombre: 'Camión Tolva 01' } },
];

// Helper function to generate mock chart data
const generateDailyData = (startDate: Date, days: number, min: number, max: number) => {
  const data = [];
...
export const mockApi = {
  ...
  createEmpresa: async (data: Omit<Empresa, 'id' | 'fechaCreacion' | 'activo'>): Promise<Empresa> => {
    await sleep(500);
  ...
  updateEmpresa: async (id: string, data: Partial<Empresa>): Promise<Empresa | undefined> => {
    await sleep(500);
  ...
  getUsuarios: async (id_empresa: string): Promise<Usuario[]> => {
    await sleep(500);
    return MOCK_USUARIOS.filter(u => u.id_empresa === id_empresa && u.habilitado);
  },
  getUsuarioById: async (id: string): Promise<Usuario | undefined> => {
    await sleep(500);
    return MOCK_USUARIOS.find(u => u.id === id && u.habilitado);
  },
  createUsuario: async (data: Omit<Usuario, 'id' | 'fechaCreacion' | 'habilitado'>): Promise<Usuario> => {
    await sleep(500);
  ...
  updateUsuario: async (id: string, data: Partial<Usuario>): Promise<Usuario | undefined> => {
    await sleep(500);
  ...
  getVehiculos: async (id_empresa: string): Promise<Vehiculo[]> => {
    await sleep(500);
    return MOCK_VEHICULOS.filter(v => v.id_empresa === id_empresa && v.activo);
  },
  getVehiculoById: async (id: string): Promise<Vehiculo | undefined> => {
    await sleep(500);
    return MOCK_VEHICULOS.find(v => v.id === id && v.activo);
  },
  createVehiculo: async (data: Omit<Vehiculo, 'id' | 'activo'>): Promise<Vehiculo> => {
    await sleep(500);
  ...
  updateVehiculo: async (id: string, data: Partial<Vehiculo>): Promise<Vehiculo | undefined> => {
    await sleep(500);
  ...
  getSurtidores: async (id_empresa: string): Promise<Surtidor[]> => {
    await sleep(500);
    return MOCK_SURTIDORES.filter(s => s.id_empresa === id_empresa && s.activo);
  },
  getSurtidorById: async (id: string): Promise<Surtidor | undefined> => {
    await sleep(500);
    return MOCK_SURTIDORES.find(s => s.id === id && s.activo);
  },
  createSurtidor: async (data: Omit<Surtidor, 'id' | 'activo'>): Promise<Surtidor> => {
    await sleep(500);
  ...
  updateSurtidor: async (id: string, data: Partial<Surtidor>): Promise<Surtidor | undefined> => {
    await sleep(500);
  ...
  createTanque: async (data: Omit<Tanque, 'id' | 'activo'>): Promise<Tanque> => {
    await sleep(500);
  ...
  updateTanque: async (id: string, data: Partial<Tanque>): Promise<Tanque | undefined> => {
    await sleep(500);
  ...
  getCentrosCosto: async (id_empresa: string): Promise<CentroCosto[]> => {
    await sleep(500);
    return MOCK_CENTROS_COSTO.filter(cc => cc.id_empresa === id_empresa && cc.activo);
  },
  getCentroCostoById: async (id: string): Promise<CentroCosto | undefined> => {
    await sleep(500);
    return MOCK_CENTROS_COSTO.find(cc => cc.id === id && cc.activo);
  },
  createCentroCosto: async (data: Omit<CentroCosto, 'id' | 'activo'>): Promise<CentroCosto> => {
    await sleep(500);
  ...
  updateCentroCosto: async (id: string, data: Partial<CentroCosto>): Promise<CentroCosto | undefined> => {
    await sleep(500);
  ...
  createEventoCarga: async (data: Omit<EventoCarga, 'id' | 'evidencias'>): Promise<EventoCarga> => {
    await sleep(500);
  ...
  // --- Alertas ---
  getAlertas: async (id_empresa: string, resuelta?: boolean): Promise<Alerta[]> => {
    await sleep(500);
  ...
  getDashboardKPIs: async (id_empresa: string) => {
    await sleep(500);
    const eventosEmpresa = MOCK_EVENTOS.filter(e => e.id_empresa === id_empresa);
    const tanquesEmpresa = MOCK_TANQUES.filter(t => t.id_empresa === id_empresa);
    const alertasEmpresa = MOCK_ALERTAS.filter(a => a.id_empresa === id_empresa);

    const totalLitros = eventosEmpresa.reduce((sum, e) => sum + e.litros, 0);
    const totalCosto = eventosEmpresa.reduce((sum, e) => sum + (e.costo_total || 0), 0);
    const avgConsumo = eventosEmpresa.length > 0 ? totalLitros / eventosEmpresa.length : 0;
    ...
    const porcentajeValidados = eventosEmpresa.length > 0 ? (eventosValidados / eventosEmpresa.length) * 100 : 0;
